    ...
    public void directValveToTray(Integer trayId) {
        ...
        for (Integer trayKey : TRAY_PIN_MAP.keySet()) {
            if (trayKey.equals(trayId)) {
                gpioController.setPin(TRAY_PIN_MAP.get(trayKey), true);
            } else {
                debugLog(format("Not dispensing to tray: %s, with pin address %s", trayKey, TRAY_PIN_MAP.get(trayKey)));
                gpioController.setPin(TRAY_PIN_MAP.get(trayKey), false);
            }
        }
    }
